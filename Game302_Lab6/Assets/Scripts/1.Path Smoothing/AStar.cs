...
public class AStar
{
    ...
    List<Node> GetSmoothedPath(List<Node> inputPath)
    {
        ...
        while (inputIndex < inputPath.Count) // Loop until we find the last item in the input
        {
            ...
            Vector2 outputVec = new Vector2(outputPath[outputPath.Count - 1].x, outputPath[outputPath.Count - 1].y);
            Vector2 inputVec = new Vector2(inputPath[inputIndex].x, inputPath[inputIndex].y);
            Vector2 dir = inputVec - outputVec;
            RaycastHit2D hit = Physics2D.Raycast(outputVec, dir, dir.magnitude);
